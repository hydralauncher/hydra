...
mod tests {
    ...
    fn compares_snapshot_hashes() {
        ...
        for (local, local_file_count, base, remote, expected) in cases {
            let result = compare_hashes(local, local_file_count, base, remote);

            assert_eq!(
                result, expected,
                "local={local:?}, local_file_count={local_file_count}, base={base:?}, remote={remote:?}",
            );
        }
    }
}

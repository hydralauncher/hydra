...
export function CloudGiftNotificationModal() {
  ...
  const openGiftPageExternal = useCallback(async (path: string) => {
    try {
      const cloudIframeUrl =
        await globalThis.window.electron.getCloudIframeUrl();
      const giftPageUrl = new URL(path, cloudIframeUrl).toString();

      await globalThis.window.electron.openExternal(giftPageUrl);
    } catch (error) {
      logger.error("Failed to open Cloud Gift page", error);
    ...
    const onOpenGiftModal = (event: Event) => {
      ...
      void globalThis.window.electron.hydraApi
        ...
        .then((giftDetails) => {
          ...
          void openGiftPageExternal(`/gifts/${giftId}`);
        })
        .catch((error) => {
          if (activeGiftIdRef.current !== giftId) return;
  ...
  const acceptGift = useCallback(async () => {
    ...
    try {
      ...
      await openGiftPageExternal(`/gifts/${giftId}`);
    } catch (error) {

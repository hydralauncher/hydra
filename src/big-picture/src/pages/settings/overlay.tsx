...
export function OverlaySettingsSection({
  ...
  const performanceAvailable = globalThis.window.electron.platform !== "linux";
  ...
  const navigationOverrides = useMemo<Record<string, FocusOverrides>>(() => {
    ...
    return {
      [ids.enabled]: {
        ...
        down: performanceAvailable
          ? { type: "item", itemId: ids.performanceEnabled }
          : { type: "block" },
      },
  ...
  return (
    ...
    >
      ...
      {performanceAvailable ? (
        ...
        <SettingsSection
          title="Performance"
          description="Use MangoHud for Linux performance metrics. Hydra does not display a duplicate performance panel."
        />
      )}
    </div>
  );

...
export function AchievementNotificationCustomizer({
  ...
  useEffect(() => {
    if (!isCustomizerOpen || parsed.status !== "absent") return;

    onCodeChange(initializeAchievementNotificationManagedCss(code));
  }, [code, isCustomizerOpen, onCodeChange, parsed.status]);

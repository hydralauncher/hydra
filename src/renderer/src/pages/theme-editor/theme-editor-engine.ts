...
const HYDRA_AUTO_TARGETS: VisualTarget[] = HYDRA_AUTO_TARGET_DATA.split(
  "\n"
).map((entry) => {
...
const isRawBackgroundImage = (value: string): boolean => {
  ...
  return (
    lower === "none" ||
    ["url(", "linear-gradient(", "radial-gradient(", "conic-gradient("].some(
...
export function generateVisualCss(document: EditorDocument): string {
  ...
  return [
    ...
    ...document.layers.flatMap((layer) =>
      renderLayer(layer, discoveredTargets)
    ),
  ].join("\n");
}
...
function stripImportantSuffix(value: string): string {
  ...
  if (
    suffixStart < 0 ||
    value.slice(suffixStart + suffix.length).trim() !== ""
  ) {
    return value;
  }
...
function mapDeclarations(
  ...
  for (const declaration of declarations) {
    ...
    if (visualProperty) {
      (mapped as Record<string, unknown>)[visualProperty] = parseCssValue(
        property,
        value
      );
    } else {
...
export function findTargetBySelector(
  ...
  for (const target of targets) {
    for (let i = 0; i < list.length; i++) {
      const score = scoreTargetCandidate(
        target.selector,
        list[i],
        list.length - i
      );

...
const terminateWithSudo = (pids: number[]) =>
...
const terminateProcesses = async (pids: number[]) => {
  ...
  const remaining = Array.from(
    new Set(failed.filter((pid) => isProcessRunning(pid)))
  );
  const terminatedWithSudo = remaining.length
    ? await terminateWithSudo(remaining)
    : false;
  return terminatedDirectly > 0 || terminatedWithSudo;
};

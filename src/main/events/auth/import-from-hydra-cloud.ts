...
const openHydraCloudImport = async (_event: Electron.IpcMainInvokeEvent) => {
  return new Promise<{ imported: number; achievements: number }>((resolve, reject) => {
    ...
    win.webContents.on("will-navigate", async (_e, url) => {
      ...
      try {
        ...
        const official = axios.create({
          ...
          headers: {
            "User-Agent": `Hydra Launcher v4.0.2`,
        ...
        logger.log(`importFromHydraCloud: done, ${games.length} games, ${totalAchievements} achievements`);
    ...
    win.on("closed", () => { if (!handled) reject(new Error("cancelled")); });
  });
};

registerEvent("openHydraCloudImport", openHydraCloudImport);

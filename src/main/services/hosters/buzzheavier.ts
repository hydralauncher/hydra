...
export class BuzzheavierApi {
  ...
  private static async getBuzzheavierDirectLink(url: string): Promise<string> {
    try {
      ...
      logger.error(`[Buzzheavier] Starting download link extraction for: ${baseUrl}`);
      ...
      logger.error(`[Buzzheavier] Making HEAD request to: ${downloadUrl}`);
      ...
      logger.error(`[Buzzheavier] Received hx-redirect header: ${hxRedirect}`);
      if (!hxRedirect) {
        logger.error(`[Buzzheavier] No hx-redirect header found. Status: ${headResponse.status}, Headers:`, headResponse.headers);
        throw new Error(
      ...
      const directLink = hxRedirect.startsWith("/dl/")
        ...
        : hxRedirect;
      logger.error(`[Buzzheavier] Extracted direct link: ${directLink}`);
      return directLink;
    } catch (error) {
  ...
  public static async getDirectLink(url: string): Promise<string> {
    logger.error(`[Buzzheavier] getDirectLink called with URL: ${url}`);
    if (!this.isSupportedDomain(url)) {
      logger.error(`[Buzzheavier] Unsupported domain: ${url}. Supported: ${this.BUZZHEAVIER_DOMAINS.join(", ")}`);
      throw new Error(

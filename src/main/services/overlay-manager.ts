...
export class OverlayManager {
  ...
  private static brokerStopTimer: NodeJS.Timeout | null = null;
  private static linuxToastActive = false;
  private static linuxTargetDisplayId: number | null = null;
  ...
  public static applyUserPreferences(preferences: UserPreferences) {
    ...
    if (
      ...
      if (this.preferences.overlayPerformanceEnabled) {
        void this.startWindowsBrokerAndPerformance(game);
      } else {
  ...
  private static async startWindowsBrokerAndPerformance(game: Game) {
    const ready = await ensureOverlayInputBroker();
    if (
      !this.servicesActive ||
      this.activeGame?.objectId !== game.objectId ||
      this.activeGame.shop !== game.shop
    ...
    const brokerActive = ready && NativeAddon.startOverlayInputBroker();
    if (this.preferences.overlayPerformanceEnabled) {
      await overlayFpsMonitor.start(game, brokerActive);
    }
  }
  ...
  public static async getContext(): Promise<HydraOverlayContext | null> {
    ...
    return {
      mode: this.linuxToastActive ? "toast" : "full",
      game: {
        title: game.title,
  ...
  private static async toggleOverlayWindow() {
    ...
    if (process.platform === "linux") {
      this.linuxTargetDisplayId = null;
      this.captureLinuxTargetDisplay();
    }
    const overlayWindow = this.ensureOverlayWindow();
    if (overlayWindow.isVisible() && !this.linuxToastActive) {
      this.hideOverlay();
      return;
    }
  ...
  public static hideOverlay() {
    ...
    const finish = () => {
      ...
      if (this.activeGame && pid) {
        setTimeout(() => {
          if (this.activeGame && this.targetPid === pid) {
            NativeAddon.focusProcessWindow(pid);
          }
        }, 25);
      }
    };
  ...
  private static async refreshTargetProcess(game: Game) {
    ...
    try {
      ...
      const nextTargetPid = candidates[0]?.pid ?? 0;
      if (nextTargetPid !== this.targetPid) this.linuxTargetDisplayId = null;
      this.targetPid = nextTargetPid;
  ...
  private static getTargetBounds(): Electron.Rectangle {
    if (process.platform === "linux") {
      const target = screen
        .getAllDisplays()
        .find((display) => display.id === this.linuxTargetDisplayId);
      return (target ?? this.captureLinuxTargetDisplay()).bounds;
    }
    if (this.targetPid) {
  ...
  private static captureLinuxTargetDisplay() {
    if (this.targetPid) {
      const processBounds = NativeAddon.getProcessWindowBounds(this.targetPid);
      if (
        processBounds &&
        processBounds.width > 0 &&
        processBounds.height > 0
      ) {
        const displays = screen.getAllDisplays();
        const display =
          findDisplayForNativeBounds(processBounds, displays) ??
          screen.getDisplayMatching(processBounds);
        this.linuxTargetDisplayId = display.id;
        return display;
      }
    }
  ...
  private static startTargetPolling() {
    ...
    this.targetPoll = setInterval(() => {
      ...
      if (this.toastWindow?.isVisible()) {
        const toastBounds = fitAuxiliaryWindow(
          bounds,
          TOAST_WIDTH,
          TOAST_HEIGHT,
          TOAST_MARGIN,
        ...
        this.toastWindow.setBounds(toastBounds);
      }
      if (this.fpsWindow?.isVisible()) {
        this.fpsWindow.setBounds(
          fitAuxiliaryWindow(bounds, FPS_WIDTH, FPS_HEIGHT, 24, "left")
        );
      }
    }, 250);
  }
  ...
  private static clearBrokerStopTimer() {
    if (this.brokerStopTimer) clearTimeout(this.brokerStopTimer);
    this.brokerStopTimer = null;
  }

  private static scheduleBrokerStop() {
    if (process.platform !== "win32") return;
    this.clearBrokerStopTimer();
    this.brokerStopTimer = setTimeout(() => {
      this.brokerStopTimer = null;
      if (!this.servicesActive) NativeAddon.stopOverlayInputBroker();
    }, 750);
  }
  ...
  private static ensureFpsWindow() {
    ...
    const targetBounds = this.getTargetBounds();
    const fpsBounds = fitAuxiliaryWindow(
      targetBounds,
      FPS_WIDTH,
      FPS_HEIGHT,
      24,
  ...
  private static registerShortcut(nativeKeyboardActive: boolean) {
    ...
    if (nativeKeyboardActive) {
      logger.info(
        `Using native ${process.platform} Hydra overlay input watcher`
      );
      return;
    }
    ...
    if (process.platform === "linux") {
      logger.warn(
        "Shift+Tab could not be registered through native Linux input; Electron portal registration was skipped"
      );
      return;
    }
  ...
  private static startControllerPolling() {
    ...
    const rawInputActive =
      ...
      NativeAddon.startOverlayKeyboardWatcher();
    if (
      !rawInputActive &&
      (process.platform === "win32" || process.platform === "linux")
    ) {
      logger.warn("Hydra native input shortcut watcher could not be started");
    }
  ...
  private static stopControllerPolling() {
    ...
    NativeAddon.stopOverlayKeyboardWatcher();

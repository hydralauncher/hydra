...
contextBridge.exposeInMainWorld("electron", {
  ...
  updateThemePreviewBounds: (themeId: string, bounds: { x: number; y: number; width: number; height: number }) =>
    ipcRenderer.invoke("updateThemePreviewBounds", themeId, bounds),
  onThemePreviewElementClicked: (cb: (selectors: string[]) => void) => {
    const listener = (_event: Electron.IpcRendererEvent, selectors: string[]) => cb(selectors);
    ipcRenderer.on("on-theme-preview-element-clicked", listener);
    return () => ipcRenderer.removeListener("on-theme-preview-element-clicked", listener);
  },

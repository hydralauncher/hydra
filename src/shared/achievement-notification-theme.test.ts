...
import {
  ...
  initializeAchievementNotificationManagedCss,
...
test("writes the default CSS values when the customizer is first opened", () => {
  const manualCss = ".library { color: red; }\n";
  const css = initializeAchievementNotificationManagedCss(manualCss);
  const parsed = parseAchievementNotificationManagedCss(css);

  assert.equal(parsed.status, "valid");
  assert.ok(css.startsWith(manualCss));
  assert.deepEqual(parsed.variations.default, {
    position: "top-left",
    scale: "1",
    background: "#1c1c1c",
    titleColor: "#c0c1c7",
  ...
  assert.deepEqual(parsed.variations.rare, { accentColor: "#f4a510" });
  assert.deepEqual(parsed.variations.hidden, {});
  assert.deepEqual(parsed.variations.platinum, {
    background: "linear-gradient(94deg, #1c1c1c -25%, #044838 100%)",
    accentColor: "#0cf1ca",
  });
});

test("does not rewrite an existing managed CSS block when opening the customizer", () => {
  const css = updateAchievementNotificationManagedCss(
    ".library { color: red; }\n",
    "rare",
    "scale",
    "1.2"
  ...
  assert.equal(initializeAchievementNotificationManagedCss(css), css);
});

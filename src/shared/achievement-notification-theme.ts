...
export const initializeAchievementNotificationManagedCss = (css: string) => {
  const parsed = parseAchievementNotificationManagedCss(css);
  if (parsed.status === "invalid") {
    throw new Error("Invalid achievement customizer markers");
  }

  if (parsed.status === "valid") return css;

  const variations = emptyManagedCss();
  variations.default = { ...DEFAULT_ACHIEVEMENT_NOTIFICATION_CSS };
  for (const [variation, overrides] of Object.entries(
    DEFAULT_VARIATION_CSS
  ) as [
    AchievementNotificationVariation,
    AchievementNotificationCssOverrides,
  ][]) {
    variations[variation] = { ...overrides };
  }
